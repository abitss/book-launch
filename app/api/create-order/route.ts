...
export async function POST(request: Request) {
  try {
    ...
    if (!book.file_path || book.purchasable === false) {
      return NextResponse.json({ error: "This ebook is not ready for instant delivery yet. Payment has been disabled until its download file is attached." }, { status: 409 });
    }
    ...
    const order = await razorpay.orders.create({ amount: Math.round(book.price * 100), currency: "INR", receipt: `ebookiee_${Date.now()}`, notes: { book_id: book.id, book_title: book.title } });

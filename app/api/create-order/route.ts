...
export async function POST(request: Request) {
  try {
    ...
    if (book.purchasable === false) return NextResponse.json({ error: "This title is listed for catalog purposes only. Checkout is unavailable until distribution rights are verified." }, { status: 403 });
